...
#>
    ...
    begin {
        Write-Verbose "[$($MyInvocation.MyCommand.Name)] Function started"
                $FileName = Get-Item -Path $InFile | Select-Object -ExpandProperty Name
    ...
    process {
        ...
        try {
            ...
            switch ($_.Exception.Response.StatusCode.Value__) {
                401 {Write-Error "Invalid Credentials."}
                403 {Write-Error "Unable to upload to TOPdesk. Make sure that you have write permissions on $ImportDirectory directory."}
                409 {Write-Error "Unknown directory on remote. Make sure that $ImportDirectory directory exists."}
                500 {Write-Error "Unknown server error."}
            }
